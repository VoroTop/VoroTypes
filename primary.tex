...
cell form a cube.  There are 11{,}648~regular triangulations, but only
152~distinct GKZ vectors and 544~star types.  The projected secondary
polytope lives in $\R^{8-4} = \R^4$ and has \textbf{74~vertices}
(out of 152 distinct GKZ points), corresponding to
\textbf{256~primary star types} out of 544~total.  Note that multiple
triangulations can share the same GKZ vector while having different
stars; all stars associated with a vertex GKZ vector are primary.

This yields a significant reduction in the combinatorial enumeration
...
\begin{tabular}{lcc}
  ...
  Primary only & 256 & $256^8 \approx 1.8 \times 10^{19}$ \\
  \hline
\end{tabular}
\end{center}
The reduction factor is approximately $4 \times 10^2$.  While less
dramatic than the FCC case, the $O_h$~point group symmetry ($|G| = 48$)
and validity pruning provide further reduction.
